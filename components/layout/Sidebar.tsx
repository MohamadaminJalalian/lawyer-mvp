...
const navItems = [
    { label: "پرونده‌ها", href: "/cases" },
    { label: "موکل‌ها", href: "/clients" },
];
                ...
                {navItems.map((item) => {
                    ...
                    return (
                        <Link
                            ...
                            href={item.href}
                            onClick={() => setIsOpen(false)}
                            className={`flex items-center gap-2 rounded-md px-2.5 py-2 text-sm ${active
                                    ? "bg-[#A9762F]/20 text-[#E7C888]"
                                    : "text-[#9BA3B4] hover:text-[#F7F5F0]"
                                }`}
                        >
                            {item.label}
                        </Link>
